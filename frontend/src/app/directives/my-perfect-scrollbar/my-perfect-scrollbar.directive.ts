...
import { MyPerfectScrollbarGeometry, MyPerfectScrollbarPosition } from './my-perfect-scrollbar.interfaces';

@Directive({
    selector: '[myPerfectScrollbar]'
})
export class MyPerfectScrollbarDirective implements OnInit, AfterViewInit, OnDestroy {
     ...
     * @param prefix
     */
    geometry(prefix: string = 'scroll'): MyPerfectScrollbarGeometry {
        return new MyPerfectScrollbarGeometry(
     ...
     * @param absolute
     */
    position(absolute: boolean = false): MyPerfectScrollbarPosition {
        if (!absolute && this.ps) {
            return new MyPerfectScrollbarPosition(
                this.ps.reach.x || 0,
                this.ps.reach.y || 0
            );
        }
